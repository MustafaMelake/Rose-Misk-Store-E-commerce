...
const Orders: React.FC = () => {
  // سحب البيانات من السياق مع تحديد النوع (Type Assertion)
  const context = useContext(ShopContext);

  if (!context) return null; // حماية في حال كان الـ Context غير متوفر

  const { currency, userOrders } = context;

  // 2. تحويل دالة المعالجة لتكون Type-Safe
  const normalizeOrderItems = (items: OrderItem[]): NormalizedItem[] => {
    if (Array.isArray(items)) {
      return items.map((it) => ({
        id: it.id,
        name: it.name,
        // نأخذ أول صورة إذا كانت مصفوفة، أو الصورة نفسها إذا كانت نصاً
